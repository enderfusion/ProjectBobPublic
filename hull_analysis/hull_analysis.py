...
# rtree is not used, but trimesh operations are crashing in the find_wind_moment function if it's not installed
import rtree
...
# wind inputs
wind_speed_average_mph = 50 # mph
## convert to in/s
wind_speed_average = (5289 / 3600) * 12 * wind_speed_average_mph # in/s
## density of air at sea level
rho =  0.0000013762 # slugs/in³
# drag coefficient
Cd = 0.42 # unitless, approximating as half sphere, unitless
...
def calculate_sliced_volume_and_centroid(stl_mesh, waterline_height, slice_direction=-1):
    # slice_direction as negative represents volume below waterline
        # Positive represents volume above waterline
    ...
    slice_plane = trimesh.intersections.slice_mesh_plane(trimesh_mesh, plane_normal=[0, slice_direction, 0], plane_origin=[0, waterline_height, 0])
    
    # Calculate the volume and centroid of the submerged part
    sliced_volume = slice_plane.volume
    sliced_centroid = slice_plane.center_mass

    return sliced_volume, sliced_centroid, slice_plane
...
def find_waterline_for_buoyancy(stl_mesh, total_mass, density_seawater=64):
    ...
    while iteration < max_iterations:
        submerged_volume, _, _ = calculate_sliced_volume_and_centroid(stl_mesh, waterline_height)
...
def find_wind_moment(wind_mesh, cod, cob, normal=[1,0,0]):
    # Pass normal from calling function with desired cross section, default is X

    # TODO: Calculate "turning" (Y axis) moment generated by wind in and out of plot
        # Will act as a parasitic power draw as delta to desired path will consume power


    # Find maximum X-Normal area
    # TODO: Moment is zeroing out at -18 degrees of rotation, it has something to do with this projection bugging
    max_area_x = trimesh.path.polygons.projected(wind_mesh, normal=[1,0,0]).area

    # Caclulate force generated by wind on this area
    wind_force = 0.5 * rho * (wind_speed_average ** 2) * Cd * max_area_x  # | slugs/in^3 | * | in^2/s^2 | * | ul | * | in^2 | = slug - in / s^2

    ## Convert to lbf
    wind_force = wind_force / 12 

    # Get distance of drag center to buoyancy center
    wind_moment_arm = cod[1] - cob[1]

    # Calculate moment generated by wind force acting on the centroid
    wind_moment = wind_moment_arm * wind_force
...
def stability_plot():
    ...
    for roll_angle in angles:
        ...
        submerged_volume, center_of_buoyancy, submerged_mesh = calculate_sliced_volume_and_centroid(rotated_mesh, waterline_height)

        # Calculate center of drag
        wind_volume, center_of_drag, wind_mesh = calculate_sliced_volume_and_centroid(rotated_mesh, waterline_height, slice_direction=1)

        # Find moment generated by wind
        wind_moment_value = find_wind_moment(wind_mesh, center_of_drag, center_of_buoyancy)
        wind_moments.append(wind_moment_value)
    ...
    df = pd.DataFrame({
        ...
        'Wind Moment (pound-inches)': wind_moments,
    ...
    scatter_righting = plt.scatter(df['Angle (degrees)'], df['Righting Moment (pound-inches)'], 
                          c=df['Color'], marker='o')
    # Plot Wind Moments as well
    scatter_wind = plt.scatter(df['Angle (degrees)'], df['Wind Moment (pound-inches)'], 
                          c='grey', marker='o')
    plt.xlabel('Angle (degrees)')
    plt.ylabel('Moment (pound-inches)')
    plt.title('Moment vs. Angle of Rotation')
    plt.grid(True)
    ...
    legend_elements = [plt.Line2D([0], [0], marker='o', color='w', label='Stable',
                       ...
                       plt.Line2D([0], [0], marker='o', color='w', label='Wind Moment',
...
def single_angle():
    ...
    # Calculate waterline and center of buoyancy for the rotated mesh
    waterline_height = find_waterline_for_buoyancy(rotated_mesh, total_mass)
    submerged_volume, center_of_buoyancy, _ = calculate_sliced_volume_and_centroid(rotated_mesh, waterline_height)
